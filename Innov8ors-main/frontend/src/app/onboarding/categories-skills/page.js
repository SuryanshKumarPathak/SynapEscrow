...
const MIN_HOURLY_RATE_INR = 250;
const MAX_HOURLY_RATE_INR = 83000;
...
export default function CategoriesAndSkillsPage() {
  ...
  const isRateValid =
    parseFloat(hourlyRate) >= MIN_HOURLY_RATE_INR &&
    parseFloat(hourlyRate) <= MAX_HOURLY_RATE_INR;
  ...
  const handleSubmit = async () => {
    ...
    if (!isRateValid) {
      setError('Please enter a valid hourly rate between ₹250 and ₹83,000.');
      return;
    }
          ...
          <section className="space-y-8">
            ...
            <div className="space-y-6 max-w-3xl">
              <div className="flex flex-col sm:flex-row sm:items-center gap-6 sm:gap-12">
                ...
                <div className="flex-1 space-y-2">
                  <label className="text-sm font-bold text-slate-800">Platform service fee</label>
              ...
              {!isRateValid && (
                <div className="flex items-center gap-2 px-4 py-3 bg-rose-50 border border-rose-100 rounded-lg text-rose-600 animate-in fade-in slide-in-from-left-2">
                  <AlertCircle size={18} />
                  <span className="text-sm font-bold tracking-tight">Enter a rate between ₹250 and ₹83,000.</span>
                </div>
              )}
            </div>
          </section>
