...
export default function FreelancerNavbar({ user }) {
  ...
  const [findWorkProjects, setFindWorkProjects] = useState([]);
  const [loadingFindWorkProjects, setLoadingFindWorkProjects] = useState(false);
  ...
  useEffect(() => {
    ...
    const loadProjects = async () => {
      try {
        setLoadingFindWorkProjects(true);
        const response = await listProjects();
        const jobs = Array.isArray(response?.data?.projects) ? response.data.projects : [];
        if (!mounted) return;
        setFindWorkProjects(jobs);
      } catch {
        if (!mounted) return;
        setFindWorkProjects([]);
      } finally {
        if (mounted) setLoadingFindWorkProjects(false);
      }
    };
              ...
              <NavItem label="Find Work" name="findWork">
                <div className="p-2">
                  ...
                  <div className="px-3 pb-1 pt-2 text-[11px] font-bold uppercase tracking-wide text-slate-400">
                    Live Employer Posts ({findWorkProjects.length})
                  </div>
                  <div className="max-h-52 space-y-1 overflow-y-auto px-1 pb-1">
                    {loadingFindWorkProjects ? (
                      <p className="px-2 py-2 text-xs text-slate-400">Loading jobs...</p>
                    ) : findWorkProjects.length === 0 ? (
                      <p className="px-2 py-2 text-xs text-slate-400">No employer posts available yet.</p>
                    ) : (
                      findWorkProjects.map((project) => (
                        <Link
                          key={String(project.id || project._id || project.postedAtTimestamp || project.name)}
                          href="/dashboard/freelancer"
                          className="block rounded-lg px-2 py-2 hover:bg-slate-50"
                          onClick={() => setOpenDropdown(null)}
                        >
                          <p className="truncate text-xs font-bold text-slate-700">{project.name || 'Untitled Project'}</p>
                          <p className="text-[11px] text-slate-500">{formatINR(project.budgetAmount)} • {project.status || 'Active'}</p>
                        </Link>
                      ))
                    )}
                  </div>
